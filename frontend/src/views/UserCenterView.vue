            ...
            <div v-else class="orders-list">
              <div v-for="order in filteredOrders" :key="order.id" class="order-card">
                ...
                <div class="order-actions">
                  ...
                  <button v-if="order.status === 'delivered'" class="action-btn action-btn-primary" @click="reorder(order)">
                    <Icon icon="mdi:refresh" />
                    再次购买
                  </button>
                  <button v-if="order.status === 'pending'" class="action-btn action-btn-primary" @click="payOrder(order)">
                    <Icon icon="mdi:credit-card" />
                    立即支付
                  </button>
                  <button v-if="order.status === 'pending'" class="action-btn action-btn-danger" @click="cancelOrder(order)">
                    <Icon icon="mdi:close" />
                    取消订单
                  </button>
                </div>
          ...
          <div class="content-header">
            ...
            <div v-else class="cart-content">
              ...
              <div class="cart-items-card">
                <!-- 分页控制 -->
                <div v-if="filteredCartItems.length > cartPageSize" class="cart-pagination">
                  <button
                    @click="prevCartPage"
                  ...
                  <span class="page-info">{{ cartCurrentPage }} / {{ cartTotalPages }}</span>
                  <button
                    @click="nextCartPage"
...
export default {
  ...
  methods: {
    ...
    // 订单操作
    reorder() {
      this.showNotification('再次购买功能开发中')
    },

    payOrder() {
      this.showNotification('跳转到支付页面')
    },
